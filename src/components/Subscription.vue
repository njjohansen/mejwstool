...
export default {
    ...
    methods: {
        ...
        getRestData () {
            this.resetMetrics()
            axios.get(this.restUrl + "/" + this.query, this.axiosConfig)
                .then(response => {
                    let result = response.data.value
                    if (result.length <= 0) return;
                    this.lastModifiedEntity = (({ Id, LastModified }) => ({ Id, LastModified }))(result.slice().sort((a, b) => a.LastModified > b.LastModified ? -1 : 1)[0])
                    this.lastMessageReceived = Date.now()
        ...
        subscribe() {
            ...
            this.connection.stream("Subscribe", payload)
                .subscribe({
                    next: (item) => {
